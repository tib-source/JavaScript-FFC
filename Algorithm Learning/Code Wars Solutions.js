...
function topThreeWords(text) {
  let regex = /[/,.\#:]+/gi
  let counter = {}
  text.trim().replace("  "," ").replace(regex, "").split(" ").forEach(word=>{
    word = word.toLowerCase()
    if ( word == "" || word.charCodeAt(0) < 97 || word.charCodeAt(0) > 122){ return undefined }
    counter[word] = (counter[word] || 0) + 1  
  })
  let answer = Object.keys(counter).sort((a,b)=>counter[b]-counter[a]).slice(0,3)
  return answer
}
